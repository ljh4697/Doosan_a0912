...
import numpy as np
from dsr_msgs.srv import OperatePytamp , OperatePytampRequest
...
class operate_joint_trajectory():
    def __init__(self):
        self.srv_operate_robot = rospy.Service("/operate_robot_joint", OperatePytamp, self.operate_robot_cb)
...
def main():
    ...
    rospy.init_node('operate_robot_joint_trajectory_py', anonymous=True)
    
    rospy.on_shutdown(shutdown)

    try:
        ojt = operate_joint_trajectory()
        rate = rospy.Rate(10)
        ...
        print("====== operate_robot_joint_trajectory is closed! ======")
